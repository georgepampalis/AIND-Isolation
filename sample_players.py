...
if __name__ == "__main__":
    ...
    player1 = AlphaBetaPlayer(score_fn=custom_score) #RandomPlayer()
    player2 = AlphaBetaPlayer(score_fn=custom_score_2) #GreedyPlayer()
    game = Board(player1, player2)

    move1 = [randint(0,6), randint(0,6)]
    move2 = move1
    while move2 == move1:
        move2 = [randint(0,6), randint(0,6)]
    ...
    print("\nWinner: Player {} ({}, {})\nOutcome: {}".format(1+(game._board_state[-3]==0), type(winner).__name__, vars(winner)['score'].__name__, outcome))
    print(game.to_string())
    history.insert(0, move2)
    history.insert(0, move1)
    print("Move history:\n{!s}".format(history))
